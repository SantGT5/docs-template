 ...
 */
export function useHeadings(containerRef: RefObject<HTMLElement | null>, key: string) {
  ...
  useEffect(() => {
    ...
    const build = () => {
      const nodes = Array.from(container.querySelectorAll<HTMLHeadingElement>("h2, h3")).filter(
        node => node.id
      )
      ...
      setActiveId(prev => (prev && nodes.some(n => n.id === prev) ? prev : nodes[0].id))
